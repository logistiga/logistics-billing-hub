...
import { Credit, Payment, formatCurrency, parseDate } from "@/components/credit-bancaire/types";
import { CreditTable } from "@/components/credit-bancaire/CreditTable";
import { PaymentsTable } from "@/components/credit-bancaire/PaymentsTable";
import { CreditDetailsDialog } from "@/components/credit-bancaire/CreditDetailsDialog";
import { CreateCreditDialog } from "@/components/credit-bancaire/CreateCreditDialog";
...
export default function CreditBancaire() {
  ...
  const upcomingPayments = credits.filter((credit) => {
    if (!credit.prochainPaiement) return false;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <Card className="border-l-4 border-l-destructive">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-destructive">{formatCurrency(totalRestant)} FCFA</div>
                <p className="text-xs text-muted-foreground">
                  {totalCapital > 0 ? ((totalRestant / totalCapital) * 100).toFixed(1) : 0}% du capital
                </p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <Card className="border-l-4 border-l-success">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-success">{formatCurrency(totalMensualites)} FCFA</div>
